// ==UserScript==
// @name         Melon Mod For Fix
// @namespace    nothing
// @version      1
// @description  auto healing at idk
// @author       ok
// @match        *://moomoo.io/*
    ...
    function (e, t, __webpack_require__) {
        ...
        function updateStatusDisplay() {
            ...
            if(player.kills > killCounter.innerText){
                killNum++;
                io.send("ch", "gg - autoGG Kill's " + killNum);
            }
            ...
            now = Date.now(), delta = now - lastUpdate, lastUpdate = now, function () {
                ...
                for (mainContext.globalAlpha = 1, mainContext.fillStyle = 'rgba(0, 0, 70, 0.35)', mainContext.fillRect(0, 0, maxScreenWidth, maxScreenHeight), mainContext.strokeStyle = darkOutlineColor, i = 0; i < players.length + ais.length; ++i)
                    if ((tmpObj = players[i] || ais[i - players.length]).visible && (10 != tmpObj.skinIndex || tmpObj == player || tmpObj.team && tmpObj.team == player.team)) {
                        ...
                        var tmpText = (tmpObj.team ? '[' + tmpObj.team + '] ' : '') + (' [' + tmpObj.sid + '/' + tmpObj.clownCounter + '/' + window.pingTime + '] ') + (tmpObj.name || ' ');
                        if ('' != tmpText) {
...
document.getElementById("gameName").innerHTML = "Melon-Mod"
...
document.getElementById('nameInput').placeholder = "Melon-Mod";
document.getElementById('chatBox').placeholder = "Chat...";
document.getElementById('diedText').innerHTML = 'U die? UwU';
document.getElementById('diedText').style.color = "Black";

$('#leaderboard').append('Melon-Mod');
...
setInterval(function() {
    if (myPlayer.hat == 45) {
        doNewSend(['ch', ['ah man freaking bad heal']]);
        hat(13);
        acc(13);
    }
}, 100);
